...
/*
  ...
  - Crie uma função quse, ao ser invocada, retorna um boolean indicando se o item  
    passado por argumento existe no array (também passado por argumento).
*/

const isItemIncluded = (item, array) => array.includes(item)
console.log(isItemIncluded(2, [2, 3, 5]))
...
showNumbersInfo = (item, index, array) => {
...
const totalPostLikes = (user = []) => {
  const firstUser = user[0]
  const secondUser = user[1]
  const thirdUser = user[2]
  const totalUsersLike = user.length - 2 
  ...
  switch(user.length) {
    ...
    case 2:
      return `${firstUser} e ${secondUser} curtiram isso.`

    case 3:
      return `${firstUser}, ${secondUser} e ${thirdUser} curtiram isso.`

    default:
      return `${firstUser}, ${secondUser} e mais ${totalUsersLike} pessoas curtiram isso.`
  }
}

// console.log(totalPostLikes([]))
// console.log(totalPostLikes(['Samuel']))
// console.log(totalPostLikes(['Samuel', 'Sebastião']))
// console.log(totalPostLikes(['Samuel', 'Sebastião', 'Simone']))
// console.log(totalPostLikes(['Samuel', 'Sebastião', 'Simone', 'Orlando']))
console.log(totalPostLikes(['Samuel', 'Sebastião', 'Simone', 'Orlando', 'Ronaldo', 'Raquel']))


// const showConsoles = console.log(postLikes())


// postLikes(['Samuel'])
// postLikes(['Samuel', 'Sebastião'])
// postLikes(['Samuel', 'Sebastião', 'Simone'])
// postLikes(['Samuel', 'Sebastião', 'Simone', 'Orlando'])
// postLikes(['Samuel', 'Sebastião', 'Simone', 'Orlando', 'Another User'])
